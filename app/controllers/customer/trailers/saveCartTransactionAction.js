...
/** 
 * 
 * @api {PUT} /cart/transaction/action Save Details of Transaction
 ...
 * 
 * 
 * @apiDescription API Endpoint to be used to save Cart Transaction Action ( Capture or Cancel ) data
 * 
 * 
...
async function saveCartTrasactionAction(req, res, next) {
    ...
    try {
        ...
        if(!cartId || !savedCart || !savedCart.transactionIdAuth) {
            return res.status(403).send({
                success: false,
                message: "Unauthorized Access"
            });
        ...
        // let cart = await Cart.findOne({ _id: cartId }); //TODO: WHY IS THIS REQUIRED??
        ...
        let errorCode = 500;
        let errors = [];
        let errorMessage = "Error while saving Cart Transaction Action ( Capture or Cancel ) data";
...
module.exports = saveCartTrasactionAction;